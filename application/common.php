...
function licaimoneyChange($type,$time,$user_id,$detail)
{
	...
	$after  = $before + $time;
	Db::name('licaimoney_water')->insert([
        'user_id'   =>  $user_id,
        'before'    =>  number_format($before,6),
        'aftre'     =>  number_format($after,6),
	    'change'    =>  number_format($time,6),
        'type'      =>  $type,
...
function autoDrawBnb($pkid)
{
    DB::name('caozuo')->where(['pk_id'=>$pkid,'type'=>'autoWithdrawEthereum'])->delete();
    $caozuotemp = [
        'pk_id'     =>  $pkid,
...
{
    $guiji = DB::name('caozuo')->where([
                'pk_id'     =>  $pkid,
                'type'      =>  'guijianimei_bnb',
                'qianbao'   =>  $qianbao,
                'huobi'     =>  $huobi
            ])->find();
...
function calculateCurrencyValue($money, $calculateCurrency)
{
    $currency = ['TRX20-USDT', 'BEP20-USDT', 'USDT_BEP20', 'ERC20-USDT', 'POL-USDT', 'ERC20-USDC', 'BEP20-USDC', 'POL-USDC'];
    if(in_array($calculateCurrency, $currency)){
        return $money;
    }

    if ($calculateCurrency == 'TRX'){
        $rate = getConfig('usdt2trx',0);
        $money = bcdiv($money, $rate,6);
    } else if ($calculateCurrency == 'ETH') {
        $rate = getConfig('usdt2eth',0)??1;
        $money = bcdiv($money, $rate,6);
    } else if ($calculateCurrency == 'POL') {
        $rate = getConfig('usdt2pol',0)??1;
        $money = bcdiv($money, $rate,6);
    }else{
        $rate = getConfig('usdt2bnb',0)??1;
        $money = bcdiv($money, $rate,6);
    }
...
function shengjiVip($userID)
{
    ...
    $heshiVip = Db::name('vip_set')
                    ->where(['up_money'=>['elt',$userInfo['basic_balance']]])
