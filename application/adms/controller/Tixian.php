...
class Tixian extends Base {
    ...
    public function settixian()
    {
        if (request()->isAjax()) {//ajax
            ...
            $tx_free_bnb = isset($postdata['tx_free_bnb'])?$postdata['tx_free_bnb']:'';
            $tx_day_bnb = isset($postdata['tx_day_bnb'])?$postdata['tx_day_bnb']:'';
            $tx_less_bnb = isset($postdata['tx_less_bnb'])?$postdata['tx_less_bnb']:'';
            $tx_bnb_rate = isset($postdata['tx_bnb_rate'])?$postdata['tx_bnb_rate']:'';
            ...
            $allowType = ['TRX20-USDT', 'BEP20-USDT', 'BEP20-USDC','ERC20-USDT', 'ERC20-USDC', 'POL-USDT', 'POL-USDC'];
            if (!empty($tikuan)) {
                foreach ($tikuan as $v) {
                    if(!in_array($v, $allowType)) {
                        htajaxReturn(0,'只支持'.implode(',', $allowType));
                    }
                }
            } else {
                htajaxReturn(0,'格式错误,只支持'.implode(',', $allowType));
            }


            if ($tixian_day_mianfei < 0 || $tx_free_bnb < 0) {
                htajaxReturn(0,'每日免手续费次数必须大于零');
            }

            if ($tixian_day_num < 0 || $tx_day_bnb < 0) {
                htajaxReturn(0,'每日次数必须大于零');
            }

            if ($tixian_rate<0||$tixian_rate>20 || $tx_bnb_rate < 0 || $tx_bnb_rate>20) {
            ...
            if (strpos($bnb_key,'********') !== false){
            } else {
                Db::name('config')->where(['config_sign'=>'tx_bnb_key'])->update(['config_value'=>$bnb_key]);
            }


            if (strpos($bep_key,'********') !== false){
            } else {
                Db::name('config')->where(['config_sign'=>'tx_bep_key'])->update(['config_value'=>$bep_key]);
            }
            ...
            // 保存BNB和BEP20 USDT提现账号
            Db::name('config')->where(['config_sign'=>'tx_bnb_address'])->update(['config_value'=>$bnb_address]);
            Db::name('config')->where(['config_sign'=>'tx_bep_address'])->update(['config_value'=>$bep_address]);

            Db::name('config')->where(['config_sign'=>'tx_free_bnb'])->update(['config_value'=>$tx_free_bnb]);
            Db::name('config')->where(['config_sign'=>'tx_day_bnb'])->update(['config_value'=>$tx_day_bnb]);
            Db::name('config')->where(['config_sign'=>'tx_less_bnb'])->update(['config_value'=>$tx_less_bnb]);
            Db::name('config')->where(['config_sign'=>'tx_bnb_rate'])->update(['config_value'=>$tx_bnb_rate]);
            ...
            $tixian_rate        = getConfig('tixian_rate',0);//提现费率
            $tixian_day_num     = getConfig('tixian_day_num',0);//每日可提现上限
            $tixian_day_mianfei = getConfig('tixian_day_mianfei',0);//免费提现次数
            ...
            // 获取BNB和BEP20 USDT提现账号
            $bnb_address    = getConfig('tx_bnb_address',0);
            $bnb_key        = getConfig('tx_bnb_key',0);
            $bep_address    = getConfig('tx_bep_address',0);
            $bep_key        = getConfig('tx_bep_key',0);


            if(strlen($bnb_key)>1){
                $bnb_key = $this->maskMiddle($bnb_key);
            }
            if(strlen($bep_key)>1){
                $bep_key = $this->maskMiddle($bep_key);
            }
            ...
            $this->assign('bnb_address',$bnb_address);
            $this->assign('bnb_key',$bnb_key);
            $this->assign('bep_address',$bep_address);
            $this->assign('bep_key',$bep_key);


            $tx_free_bnb        = getConfig('tx_free_bnb',0);
            $tx_day_bnb         = getConfig('tx_day_bnb',0);
            $tx_less_bnb        = getConfig('tx_less_bnb',0);
            $tx_bnb_rate        = getConfig('tx_bnb_rate',0);

            $this->assign('tx_free_bnb',$tx_free_bnb);
            $this->assign('tx_day_bnb',$tx_day_bnb);
            $this->assign('tx_less_bnb',$tx_less_bnb);
            $this->assign('tx_bnb_rate',$tx_bnb_rate);

            return view();
        }
    }

    public function maskMiddle($secret) {
        $len = strlen($secret);
