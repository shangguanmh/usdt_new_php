...
// 			Db::name('config')->where(['config_sign'=>'kefufeiji'])->update(['config_value'=>$kefufeiji]);
    ...
    function huilv(){
        if(request()->isAjax()){//ajax
            ...
            $usdt2eth 	= isset($postdata['usdt2eth'])?$postdata['usdt2eth']:'';
            $usdt2pol 	= isset($postdata['usdt2pol'])?$postdata['usdt2pol']:'';
            ...
            $isHas = Db::name('config')->where(['config_sign'=>'usdt2trx'])->count();
            if($isHas == 0){
                Db::name('config')->insert(['config_sign'=>'usdt2trx','config_value'=>$usdt2trx]);
            }else{
                Db::name('config')->where(['config_sign'=>'usdt2trx'])->update(['config_value'=>$usdt2trx]);
            }

            $isHas = Db::name('config')->where(['config_sign'=>'usdt2bnb'])->count();
            if($isHas == 0){
                Db::name('config')->insert(['config_sign'=>'usdt2bnb','config_value'=>$usdt2bnb]);
            }else{
                Db::name('config')->where(['config_sign'=>'usdt2bnb'])->update(['config_value'=>$usdt2bnb]);
            }

            $isHas = Db::name('config')->where(['config_sign'=>'usdt2eth'])->count();
            if($isHas == 0){
                Db::name('config')->insert(['config_sign'=>'usdt2eth','config_value'=>$usdt2eth]);
            }else{
                Db::name('config')->where(['config_sign'=>'usdt2eth'])->update(['config_value'=>$usdt2eth]);
            }

            $isHas = Db::name('config')->where(['config_sign'=>'usdt2pol'])->count();
            if($isHas == 0){
                Db::name('config')->insert(['config_sign'=>'usdt2pol','config_value'=>$usdt2pol]);
            }else{
                Db::name('config')->where(['config_sign'=>'usdt2pol'])->update(['config_value'=>$usdt2pol]);
            }
            htajaxReturn(1,'修改成功');
        }else{
            $usdt2trx = getConfig('usdt2trx',0);
            $usdt2bnb = getConfig('usdt2bnb',0);
            $usdt2eth = getConfig('usdt2eth',0);
            $usdt2pol = getConfig('usdt2pol',0);

            $this->assign('usdt2trx',$usdt2trx);
            $this->assign('usdt2bnb',$usdt2bnb);
            $this->assign('usdt2eth',$usdt2eth);
            $this->assign('usdt2pol',$usdt2pol);
