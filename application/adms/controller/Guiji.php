...
class Guiji extends Base {
    ...
    public function bnbuseraddress(){
    	//*********************************************整理展示字段
    	$showCol = [
    		['col'=>'address','chinaname'=>'地址','style'=>''],
    		['col'=>'yonghu','chinaname'=>'充值用户','style'=>''],
    		['col'=>'usdt','chinaname'=>'USDT余额','style'=>''],
    		['col'=>'bnb','chinaname'=>'BNB余额','style'=>''],
    		['col'=>'chongzhijilu','chinaname'=>'充值记录','style'=>''],
    	];
    	//*********************************************整理展示字段
    ...
    // 		'address'=>['chinaname'=>'地址','ca'=>'like','type'=>'text','style'=>''],
    	...
    	$where['user_id'] = ['neq',0];
    	//***********************************************整理搜索字段
   	 	$list = Db::name('bnb_address')->where($where)->order('user_id desc')->paginate(15);//查询数据
   	 	$page = isset($this->params['page'])?$this->params['page']:1;
   	 	$pageInfo = setPage($list->total(),$page);
   	 	$list =  $list->all(); //分页对象转化为数组
   	 	foreach($list as $k=>$v){
    ...
    public function bnbrecord(){
    	...
    	$where['status'] = 1;
    	//***********************************************整理搜索字段
   	 	$list = Db::name('bnb_guiji_record')->where($where)->order('add_time desc')->paginate(15);//查询数据
   	 	$page = isset($this->params['page'])?$this->params['page']:1;
   	 	$pageInfo = setPage($list->total(),$page);
   	 	$list =  $list->all(); //分页对象转化为数组
   	 	foreach($list as $k=>$v){
   	 		$domain = 'https://bscscan.com';
            
            $list[$k]['from_address'] = '<a class="dizhi" href="'.$domain.'/address/'.$v['from_address'].'" target="_blank">'.$v['from_address'].'</a>';
			$list[$k]['to_address'] ='<a class="dizhi" href="'.$domain.'/address/'.$v['to_address'].'" target="_blank">'.$v['to_address'].'</a>';
 			$list[$k]['status'] = $v['status'] == 0 ? fontcolor('未完成','red') : fontcolor('已完成','green');
 			$list[$k]['money'] = '<b>'.$list[$k]['money'].'<b>';
    ...
    public function yijianbnbguiji(){
    	$list = DB::name('bnb_guiji_record')->where(['status'=>0])->select();
    	foreach($list as $val){
    		$this->createCollectionTask($val['from_address'], $val['huobi'], 0);
    		DB::name('bnb_guiji_record')->where(['id'=>$val['id']])->delete();
    	}
		htajaxReturn(1,'操作成功，重新发起BNB归集'.count($list).'条');
    }
