			...
			foreach ($caozuo as $val) {
				...
				switch($type) {
					case 'licai'://理财到期
						...
						$licaiIssue = Db::name('user_licai_issue')->where([
                            'issue'         =>  $extra['issue'],
						    'user_licai_id' =>  $extra['user_licai_id'],
                            'status'        =>  1
                        ])->find();
					...
					case 'updateXiaxian'://更新下线数量
						...
						if (!empty($yiji)) {
							$erji_from  = $yiji['from_who'];//二级
							$sanji_from = $yiji['erji_from'];//三级

							Db::name('user')->where(['id'=>$val['pk_id']])->update([
                                'erji_from'  => $erji_from,
                                'sanji_from' => $sanji_from
                            ]);
					...
					case 'chongzhiticheng'://充值的提成
						...
						if (!empty($invest_order)) {
							...
							$user = Db::name('user')
                                    ...
                                    ->field('user.id,user.vip_level,user1.id as user_id1,user2.id as user_id2,user3.id as user_id3,user1.vip_level as vip_level1,user2.vip_level as vip_level2,user3.vip_level as vip_level3')
                                    ->find();
							...
							if (!empty($user)) {
								...
								if (!empty($user['user_id1'])) {
				 					...
				 					//充值返利
                                     Db::name('chognzhifanli')->insert([
                                         'user_id'      =>  $user['user_id1'],
                                         'czjine'       =>  $invest_order['zuihou_value'],
                                         'money'        =>  $yiji_ticheng,
                                         'add_time'     =>  date('Y-m-d H:i:s'),
				 					     'vip_level'    =>  $user['vip_level1'],
                                         'rate'         =>  $rate1_chongzhi,
                                         'laizi_user_id'=>  $invest_order['user_id'],
                                         'jibie'        =>  1
                                     ]);
								}
								...
								if (!empty($user['user_id3'])) {
									...
									$sanji_ticheng  = bcmul($invest_order['zuihou_value'],bcmul($rate3_chongzhi,0.01,6),6);
				 					...
				 					Db::name('chognzhifanli')->insert([
                                        'user_id'       =>  $user['user_id3'],
                                        'czjine'        =>  $invest_order['zuihou_value'],
                                        'money'         =>  $sanji_ticheng,
                                        'add_time'      =>  date('Y-m-d H:i:s'),
                                        'vip_level'     =>  $user['vip_level3'],
                                        'rate'          =>  $rate1_chongzhi,
                                        'laizi_user_id' =>  $invest_order['user_id'],
                                        'jibie'         =>  3
                                    ]);
                                ...
                                foreach($result['data'] as $record) {
                                    if (isset($record['ret'][0]['contractRet'])
                                        ...
                                        if ($block_timestamp == $last_time||$lasthash_id ==$txID) {//匹配到一样的交易，就结束了
                                            echo '匹配到一样的交易，就结束了trx'.$block_timestamp.'  '.$txID;
                                            redisCache()->del($lock_key);
                                            break;
                                        }
                                }
					...
					case 'autotixian':
                        ...
                        $tixian_trx     = $config['tixian_trx'];//提现账户trx余额
                        $tixian_usdt    = $config['tixian_usdt'];//提现账户usdt余额
                        $tixian_Energy  = $config['tixian_energy'];//提现账户能量余额
	                     
                        $tixian = Db::name('draw_order')->where([
                            'id'        =>  $val['pk_id'],
                            'huobi'     => 'TRX20-USDT',
                            'status'    =>  1
                        ])->find();
	                     ...
	                     if (isset($tixian['huobi'])
                             && isset($tixian_Energy)
                             && $tixian['huobi'] == 'TRX20-USDT'
                         ) {
						 ...
						 if (!empty($tixian)) {
							...
							Db::name('draw_order')->where(['id'=>$tixian['id']])->update([
                                'tranfer_detail'    =>  json_encode($result),
                                'status'            =>  2,
							    'shenhe_user_id'    =>  $shenheuid,
				...
				foreach ($userList as $user) {
                    ...
                    if ($lixi > 0) {
				    	$cunzai = Db::name('wakuang')
				    	                ->where(['user_id'=>$user['id'],'daoqi_day'=>$jintian])
                                        ->find();
	...
	function guijitask()
    {
		...
		foreach ($list as $val) {
			...
			if ($token == 'TRX20-USDT') {
				...
				$usdtYue = $this->getBalannce($address,'TRX20-USDT');
...
//							echo '</br>转账结果：'.json_encode($zhuanzhangResult);
				    ...
				    if ($energy < 64400 && $tixian_trx >=2 && $trxyue >= $zuidizhi) {
                        $orderId = $val['pk_id'];
                        echo("\n归集能量不够，购买能量。 \n");
	...
	function test(){
	    echo $this->getEnergy('TKf4aEj5pJzEJWrCSwkdzSsYmnHzJbtibM');
	}
	function test11a(){
		$this->daozhang('TKf4aEj5pJzEJWrCSwkdzSsYmnHzJbtibM',5,'USDT',1049,'8a3eb05d542ed25c35b4');
	}
	function gettestre(){
	    $url = "https://api.trongrid.io/v1/accounts/TKf4aEj5pJzEJWrCSwkdzSsYmnHzJbtibM/transactions/trc20?only_to=true&limit=8";
	    $resu = http_get($url);
	    echo $resu;
	}
}
